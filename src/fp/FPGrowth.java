...
public class FPGrowth {
    ...
    Vector<FPtree> tempFPTree = new Vector<FPtree>();
    FPtree fptree;
    //fp-growth
    ...
    private FPtree conditional_fptree_constructor(Map<String, Integer> conditionalPatternBase, Map<String, Integer> conditionalItemsMaptoFrequencies, int threshold, Vector<FPtree> conditional_headerTable) {
        //FPTree constructing tao cay dieu kien
        //the null node!
    	//conditionalPatternBase la khong gian mau co dieu kien chua 
    	//conditionalItemsMaptoFrequencies la tap item 1 phan tu de so sanh voi threshold
    ...
    private void fptree(File file) throws FileNotFoundException {
        ...
        //Scanner input = new Scanner(file);//1
        Scanner input = null;
    ...
    private void preProcessing(File file, Map<String, Integer> itemsMaptoFrequencies, Scanner input, List<String> sortedItemsbyFrequencies, Vector<String> itemstoRemove) throws FileNotFoundException {
        ...
        //Save input data in fisrt scan databases.
        while (input.hasNextLine()) {
            //Read transaction from input data
            ...
            //Add transaction into global variable arrData
            arrData.add(tempLine);
...
//
////            for (String vectorString : transactionSortedbyFrequencies) {
////            System.out.printf("%s:%s ", vectorString, itemsMaptoFrequencies.get(vectorString));
////            }
////            System.out.println();
//            //adding to tree
...
//        }
        ...
        fptree.item = null;
        fptree.root = true;
        for (ArrayList<String> line : arrData) {
            int sizeTransaction = line.size();
            Vector<String> transactionSortedbyFrequencies = new Vector<String>();
...
//            fptree.children = new Vector<FPtree>(tempFPTree);
//        }
//        int tempIndex = Integer.parseInt(itemtoAddtotree);
//        FPtree tempNode = fptree.children.get(tempIndex);
//        if (tempNode.item.compareTo("") != 0) {
//            newNode = tempNode;
//            tempNode.count++;
//            ifisdone = true;
//            fptree.children.set(tempIndex, tempNode);
//        }
        if (!ifisdone) {
            ...
            //fptree.children.set(Integer.parseInt(newNode.item), newNode);
...
//            System.out.print(currentPattern + " ");
...
//                System.out.print(supportofCurrentPattern +" a ");
//                System.out.print(iteminTree.parent.item + " b ");
...
//            System.out.println();
    ...
    private void print() throws FileNotFoundException {
         ...
         * 
         */
        Formatter output = new Formatter("result_fpgrpwth.dat");
        countFreItem = 1;
        for (String frequentPattern : frequentPatterns.keySet()) {
            output.format("%s\t%d\n", frequentPattern, frequentPatterns.get(frequentPattern));
            //System.out.print("Index" + i + "," + frequentPattern + "\t" + frequentPatterns.get(frequentPattern) + "\n");
            countFreItem++;
        }
    }
